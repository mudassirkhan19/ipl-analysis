...
HighBids <- head(combined_stats[,.(PLAYER,TEAM,YEAR,PRICE.PAID,Points,PricePerPoint)][order(-PRICE.PAID)],15)
...
The analysis is done only for the year the player has been bought and doesn't include subsequent years' performance of the player if retained by the team.
...
Yuvraj Singh features in three of the top 10 bids as was the most paid cricketer in IPL for 2014 & 2015. DD and RCB are among the big spenders as they hope to get the best players for their team. This makes sense as they have struggled to find performing players in their team and usually don't retain big players as their big bets haven't paid off yet.
...
g2 + xlab("Total Spending (In Crores)") + ylab("MVP Points") + theme(axis.title = element_text(size = 12), axis.title.y = element_text(margin=margin(0,10,0,0)))
```
